# Changing pipeline to use undistorted images
...
# TODO: calibrate with full size image, allow scaling
...
class RobotCoordinateConverter:
    ...
    def __init__(self, camera_calibration_file=None):
        ...
        with np.load(camera_calibration_file) as calibration_values:
            self.cam_mtx, self.dist, self.calib_shape = [calibration_values[i] for i in ('mtx', 'dist', 'shape')]
            self.rvec = None
            self.tvec = None
    ...
    def undistort(self, img, debug=False):
        ''' Undistorts img using instance calibration values'''
        if img.shape[0] != self.calib_shape[0] and img.shape[1] != self.calib_shape[1]:
            raise ValueError("Image shape {} does not equal calibration shape {}, scaling not implemented yet".format(img.shape[:2], self.calib_shape[:2]))
        print(self.cam_mtx)
        print(self.dist)
        print(self.new_cam_mtx)
        # new_cam_mtx does not change undistorted img
        #undistorted_img = cv2.undistort(img, self.cam_mtx, self.dist, self.new_cam_mtx)
        undistorted_img = cv2.undistort(img, self.cam_mtx, self.dist)
        print(undistorted_img.shape)
        print(img.shape)
        diff = np.subtract(img, undistorted_img)
        if debug:
            #cv2.rectangle(undistorted_img, self.roi[:2], self.roi[2:4], (0, 255, 0), 2)
    ...
    def calibrate(self, img, debug=False):
        ...
        if ret:
            ...
            h,  w = img.shape[:2]
            self.new_cam_mtx, self.roi = cv2.getOptimalNewCameraMatrix(self.cam_mtx,self.dist,(w,h),1,(w,h))
            undistorted_img = self.undistort(img, True)
            # print("rvec: {}".format(self.rvec))
            # 1x2 points
            #imgpts, jac = cv2.projectPoints(axis, self.rvec, self.tvec, self.cam_mtx, self.dist)
            # tested, do not want to use new cam mtx
            imgpts, jac = cv2.projectPoints(axis, self.rvec, self.tvec, self.cam_mtx, None)
            print(imgpts)
    ...
    def robotToPixel(self, robot_coord, round_vals=True, debug=False):
        '''
        Converts from robot coordinate to image coordinate for undistorted image
        ...
        img_pts, _ = cv2.projectPoints(np.array([obj_point]), self.rvec, self.tvec, self.cam_mtx, None)
        #img_pts, _ = cv2.projectPoints(np.array([obj_point]), self.rvec, self.tvec, self.new_cam_mtx, None)
        #print(img_pts)
...
if __name__ == "__main__":
    ...
    img = cv2.imread('images/testset1/checkerboard_img_w1000_0.jpg')
    converter.calibrate(img, debug=True)
    img = cv2.imread('images/testset1/checkerboard_img_w1000_0.jpg')
    img = converter.undistort(img, True)
    # cv2.imshow("img", img)
    # cv2.imshow("img2", img2)
    # print(np.subtract(img, img2))
    # cv2.waitKey(0)
    ...
    #             p1 = converter.robotToPixel((146 + space * i, 227 - space * j, converter.checkerboard_z + k * space))
    #             #p2 = converter.robotToPixel((146 + space * i, 227 - space * j, 8))
    #             p3 = converter.robotToPixel((146 + space * (i + 1), 227 - space * j, converter.checkerboard_z + k * space))
    #             p4 = converter.robotToPixel((146 + space * i, 227 - space * (j + 1), converter.checkerboard_z + k * space))
    #             # a1
    #             # p5 = converter.robotToPixel((146 + space * i + 11, 227 - space * j + 14, converter.checkerboard_z + space))
    #             p6 = converter.robotToPixel((146 + space * i, 227 - space * j, converter.checkerboard_z + space * (k+1)))
    #             p7 = converter.robotToPixel((146 + space * (i + 1), 227 - space * j, converter.checkerboard_z + space * (k+1)))
    #             p8 = converter.robotToPixel((146 + space * i, 227 - space * (j + 1), converter.checkerboard_z + space * (k+1)))
    ...
    print(converter.pixelToRobot((411, 314), -41.5))
    print(converter.pixelToRobot(converter.robotToPixel((40, 400, -41.5)), -41.5))