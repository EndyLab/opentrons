...
def compress(img, factor):
	h, w = img.shape[:2]
	print(h,w)
	mat = np.ones((factor,factor),np.uint8)
	# (h,w)
	compressed_mat = np.zeros((h//factor, w//factor))
	for x in range(0,w-factor,factor):
		for y in range(0,h-factor,factor):
			val = np.zeros((factor, factor))
			# construct 3x3 matrix of pixel values
			crop = img[y:y+factor,x:x+factor]
			res = np.bitwise_and(mat,crop)
			nonzero = np.count_nonzero(res)
			if nonzero > 5:
				# white
				compressed_mat[y//factor][x//factor] = 255
			else:
				# blacki
				compressed_mat[y//factor][x//factor] = 0
